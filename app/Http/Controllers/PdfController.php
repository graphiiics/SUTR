	...
	public function registroPdf(Registro $registro)
	{
		...
		////////////////// tabla inicio
			...
			foreach ($registro->productos as $key => $producto) {
				...
				Fpdf::Cell(30,6,$key+1,1,0,'C');
				Fpdf::Cell(60,6,utf8_decode($producto->nombre),1,0,'L');
				Fpdf::Cell(45,6,$producto->categoria,1,0,'L');
				if($producto->pivot->cantidad==1)
					Fpdf::Cell(45,6,$producto->pivot->cantidad.' '.$producto->presentacion,1,0,'L');
				else
					Fpdf::Cell(45,6,$producto->pivot->cantidad.' '.$producto->presentacion.'s',1,0,'L');
				Fpdf::Ln();
